...
// Import layout component to wrap the page content
import DashboardLayout from '@/components/DashboardLayout';
// Import TypeScript types and constants for type safety and consistency
...
// Define an initial empty state for a barangay to reset the form easily
...
// Import next/dynamic for dynamic importing of components (lazy loading)
import dynamic from 'next/dynamic';

// Dynamically import the LocationPicker component, disabling Server-Side Rendering (ssr: false)
// because map components usually rely on browser APIs like window/document.
// A fallback loading UI is shown while the component loads.
...
export default function BarangaysPage() {
  ...
  const [barangays, setBarangays] = useState<Barangay[]>([]); // Stores the list of barangays fetched from the database
  const [search, setSearch]       = useState('');             // Stores the current search query
  const [modal, setModal]         = useState<'add' | 'edit' | null>(null); // Controls which modal is open ('add' or 'edit'), null means closed
  const [editing, setEditing]     = useState<Barangay | null>(null);       // Stores the barangay currently being edited
  const [form, setForm]           = useState({ ...EMPTY });   // Stores the data of the form being filled out
  const [contactsStr, setContactsStr] = useState('');         // Stores contact numbers as a single comma-separated string for the input field
  const [saving, setSaving]       = useState(false);          // Indicates if a save operation is in progress (for loading states)
  const [error, setError]         = useState('');             // Stores error messages to display to the user
  
  // Initialize the Supabase client
  const supabase = createClient();

  // Function to fetch the list of barangays from the Supabase database
  // Wrapped in useCallback to prevent unnecessary re-creations of this function on every render
  const fetchBarangays = useCallback(async () => {
    // Query the 'barangays' table, selecting all columns and ordering by 'name'
    const { data } = await supabase.from('barangays').select('*').order('name');
    if (data) setBarangays(data as Barangay[]); // Update state if data is fetched successfully
  }, [supabase]);

  // Fetch the data when the component mounts
  useEffect(() => { fetchBarangays(); }, [fetchBarangays]);

  // Function to prepare and open the 'Add Barangay' modal
  const openAdd = () => {
    setForm({ ...EMPTY }); // Reset the form to empty state
    setContactsStr('');    // Clear the contacts string
    setEditing(null);      // Ensure no barangay is set as currently editing
    setError('');          // Clear any previous errors
    setModal('add');       // Open the 'add' modal
  };

  // Function to prepare and open the 'Edit Barangay' modal with data from the selected barangay
  const openEdit = (b: Barangay) => {
    // Populate the form state with the selected barangay's data
    ...
    // Convert the array of contact numbers into a comma-separated string for the input field
    setContactsStr(b.contact_numbers.join(', '));
    setEditing(b);         // Set the selected barangay as the one being edited
    setError('');          // Clear any previous errors
    setModal('edit');      // Open the 'edit' modal
  };

  // Function to handle saving (both adding new and updating existing) a barangay
  const handleSave = async () => {
    ...
    const payload = {
      ...form,
      // Convert the comma-separated string back into an array of trimmed strings, removing empty ones
      contact_numbers: contactsStr.split(',').map(s => s.trim()).filter(Boolean),
    };

    // Determine the API endpoint URL based on whether we are adding or editing
    const url    = modal === 'edit' ? `/api/barangays/${editing!.id}` : '/api/barangays';
    // Determine the HTTP method (PUT for updating, POST for creating)
    ...
    // On success: stop loading, close modal, and refresh the list
    setSaving(false); 
    setModal(null); 
    fetchBarangays();
  };

  // Function to handle deleting a barangay
  ...
  // Filter the barangays list based on the search input (matching name or captain name, case-insensitive)
  ...
  return (
    // Wrap the entire page in the DashboardLayout for consistent navigation/styling
    <DashboardLayout>
      <div className="animate-fade-up">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.75rem', flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            {/* Display the total count of barangays */}
        ...
        <div className="glass-card" style={{ overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto' }}>
            <table className="data-table">
              ...
              <tbody>
                {/* Display empty state message if no barangays match the search */}
                {filtered.length === 0 && (
                  <tr><td colSpan={6} style={{ textAlign: 'center', padding: 32, color: 'var(--muted)' }}>No barangays found</td></tr>
                )}
                {/* Render a row for each filtered barangay */}
                {filtered.map(b => {
                  const rc = RISK_COLORS[b.risk_level]; // Get the color associated with the risk level
                  return (
                    <tr key={b.id}>
      ...
      {/* Modal Section (Rendered only when 'modal' state is not null) */}
      {modal && (
      ...
      {/* Global style definition for the spinner animation */}
